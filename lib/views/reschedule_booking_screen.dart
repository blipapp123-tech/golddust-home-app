...
class _RescheduleBookingScreenState extends State<RescheduleBookingScreen> {
  ...
  Future<void> _fetchAvailableSlots(DateTime date) async {
    ...
    final selectedDay = _dayName(date);
    final cacheKey = selectedDay;
    ...
    try {
      ...
      final assignedMaliSearchName =
      _normalizeMaaliSearchName(assignedMaliName);

      final summaryResponse = await http.get(
        Uri.parse(_availabilityTrackerUrl),
      ...
      if (summaryResponse.statusCode != 200) {
        throw Exception('Availability API failed');
      }

      final summaryData = jsonDecode(summaryResponse.body);

      if (summaryData['success'] != true) {
        throw Exception(summaryData['message'] ?? 'Unable to fetch availability');
      }

      final List summaryRows =
          summaryData['totalRows'] ?? summaryData['rows'] ?? [];

      final List<dynamic> maaliNames = summaryData['maaliNames'] ?? [];

      String? matchedMaaliName;

      if (assignedMaliSearchName.isNotEmpty) {
        for (final name in maaliNames) {
          final dbName = name.toString().trim().toLowerCase();

          if (dbName.contains(assignedMaliSearchName) ||
              assignedMaliSearchName.contains(dbName)) {
            matchedMaaliName = name.toString().trim();
            break;
          }
        }
      }
      ...
      if (matchedMaaliName != null && matchedMaaliName.isNotEmpty) {
        final assignedUrl =
            '$_availabilityTrackerUrl?maaliName=${Uri.encodeComponent(matchedMaaliName)}';
        ...
        if (assignedResponse.statusCode == 200) {
          ...
          if (assignedData['success'] == true) {
            assignedRows = assignedData['rows'] ?? [];
          }
        }
      }
      ...
      for (final row in assignedRows) {
        final count = _getAvailabilityCount(row, selectedDay);
        if (count <= 0) continue;

        final slot = (row['displayTimeSlot'] ??
            row['timeSlot'] ??
            row['slot'] ??
            '')
            .toString()
        ...
        assignedSlots.add({
          'slot': slot,
          'available': true,
          'maaliId': widget.assignedMaliId,
          'maaliName': matchedMaaliName ?? assignedMaliName,
      ...
      for (final row in summaryRows) {
        final totalCount = _getAvailabilityCount(row, selectedDay);
        ...
        final assignedCountForSameSlot = assignedSlots
            ...
            .fold<int>(
          0,
              (sum, e) => sum + ((e['availableCount'] ?? 0) as int),
        );

        final otherCount = totalCount - assignedCountForSameSlot;
        ...
        otherSlots.add({
          'slot': slot,
          'available': true,
          'availableMaaliCount': otherCount,
          'maaliId': '',
          'maaliName': '',
      ...
      debugPrint('❌ Availability tracker fetch error: $e');
  ...
  Future<void> _submitReschedule() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _errorMessage =
          'Failed to reschedule booking. Server responded with: ${response.body}';
        });

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to reschedule: ${response.body}'),
            backgroundColor: Colors.red,
          ),
        );
      }
